...
async function discoverMovie() {
  ...
  try {
    ...
    const [detailRes, videoRes] = await Promise.all([
      ...
      fetch(`${BASE}/movie/${picked.id}/videos?api_key=${API_KEY}&language=en-US`)
    ]);
    ...
    setTimeout(() => {
      ...
      if (err.message === 'no_results' || err.message === 'no_poster') {
        ...
        showError('Koneksi ke gudang film gagal. Cek API key atau coba lagi sebentar. 📡');
      }
    }, delay);
  }
}
...
function renderResult(movie) {
  ...
  const langLabel = movie.original_language ? movie.original_language.toUpperCase() : '';
...
function openModal(key) {
  if (key) {
    ...
    trailerFrame.src = '';
    trailerFrame.insertAdjacentHTML('afterend', '<p class="no-trailer">Trailer tidak ditemukan untuk film ini 🎬</p>');
  }
